...
(() => {
  ...
  const DESKTOP_MASCOT_MODE = OBS_MODE && URL_PARAMS.get("desktop") === "1";
  const OBS_PRESETS = {
  ...
  const ASSETS = {
    backHair: "assets/amber-avatar/back-hair.png",
    frontHair: "assets/amber-avatar/front-hair.png",
    eyesOpenMouthClosed: "assets/amber-avatar/eyes-open-mouth-closed.png",
    eyesOpenMouthHalf: "assets/amber-avatar/eyes-open-mouth-half.png",
    eyesOpenMouthOpen: "assets/amber-avatar/eyes-open-mouth-open.png",
    eyesClosedMouthClosed: "assets/amber-avatar/eyes-closed-mouth-closed.png",
    eyesClosedMouthHalf: "assets/amber-avatar/eyes-closed-mouth-half.png",
    eyesClosedMouthOpen: "assets/amber-avatar/eyes-closed-mouth-open.png",
  };
  ...
  const BRONZE_AVATAR_ASSETS = {
    backHair: "assets/bronze-avatar/back-hair.png",
    frontHair: "assets/bronze-avatar/front-hair.png",
    eyesOpenMouthClosed: "assets/bronze-avatar/eyes-open-mouth-closed.png",
    eyesOpenMouthHalf: "assets/bronze-avatar/eyes-open-mouth-half.png",
    eyesOpenMouthOpen: "assets/bronze-avatar/eyes-open-mouth-open.png",
    eyesClosedMouthClosed: "assets/bronze-avatar/eyes-closed-mouth-closed.png",
    eyesClosedMouthHalf: "assets/bronze-avatar/eyes-closed-mouth-half.png",
  ...
  const OPTIONAL_AVATAR_EXPRESSION_ASSETS = {
    emotionHappyMouthClosed: "avatar/emotion-happy-mouth-closed.png",
    emotionHappyMouthHalf: "avatar/emotion-happy-mouth-half.png",
    emotionHappyMouthOpen: "avatar/emotion-happy-mouth-open.png",
    emotionSurprisedMouthClosed: "avatar/emotion-surprised-mouth-closed.png",
    emotionSurprisedMouthHalf: "avatar/emotion-surprised-mouth-half.png",
    emotionSurprisedMouthOpen: "avatar/emotion-surprised-mouth-open.png",
    emotionSoftMouthClosed: "avatar/emotion-soft-mouth-closed.png",
    emotionSoftMouthHalf: "avatar/emotion-soft-mouth-half.png",
    emotionSoftMouthOpen: "avatar/emotion-soft-mouth-open.png",
  };
  ...
  async function ensureBronzeAvatarCharacterProfile() {
    if (autoCharacterSourceWasDeleted(BRONZE_AVATAR_SOURCE_KIND)) return false;
    const profiles = await listCharacterProfiles();
    if (profiles.some((profile) => managedDemoAvatarSourceKindForProfile(profile) === BRONZE_AVATAR_SOURCE_KIND)) return false;

    const record = await buildCharacterProfileRecordFromAssetMap({
      assetMap: BRONZE_AVATAR_ASSETS,
      name: "キャラ4",
      sourceKind: BRONZE_AVATAR_SOURCE_KIND,
      settingsUrl: BRONZE_AVATAR_SETTINGS_URL,
    });
    await putCharacterProfile(record);
    return true;
  }
  ...
  function applyObsExternalExpression(nowMs) {
    if (!OBS_MODE || nowMs >= obsExternalInput.expressionUntil) {
      if (nowMs >= obsExternalInput.expressionUntil) {
    ...
    if (Number.isInteger(obsExternalInput.forceMouth)) {
      const nextMouth = clamp(obsExternalInput.forceMouth, 0, 2);
      startMouthCrossfade(nextMouth, nowMs);
      mouthState = nextMouth;
      previousMouthState = nextMouth;
    }
    if (typeof obsExternalInput.forceEyesClosed === "boolean") {
      blinkClosed = obsExternalInput.forceEyesClosed;
    }
  }
  ...
  async function applyObsSnapshot(snapshot) {
    ...
    if (snapshot.avatarImages && typeof snapshot.avatarImages === "object") {
      ...
      for (const key of Object.keys(OPTIONAL_AVATAR_EXPRESSION_ASSETS)) delete images[key];
      const optionalKeys = Object.keys(OPTIONAL_AVATAR_EXPRESSION_ASSETS).filter((key) => snapshot.avatarImages[key]);
      const optionalSettled = await Promise.all(optionalKeys.map(async (key) => {
        const image = await loadImageFromDataUrl(String(snapshot.avatarImages[key]), key);
        validateAvatarImageDimensions(image, key, expectedAvatarSize);
        return [key, image];
      }));
      for (const [key, image] of optionalSettled) loaded[key] = image;
      applyLoadedAvatarImages(loaded);
    }
  ...
  function updateMouseFollowButton() {
    ...
    setButtonLabel(ui.mouseFollowButton, idleLocksMouse ? "マウス追従を開始" : `マウス追従 ${state.mouseFollowEnabled ? "ON" : "OFF"}`);
    ui.mouseFollowButton.setAttribute("aria-pressed", String(!idleLocksMouse && state.mouseFollowEnabled));
    // 待機ゆれ中でも押せるようにする。クリック時に待機ゆれを解除して
    // マウス追従へ切り替えるため、disabled にはしない。
    ui.mouseFollowButton.disabled = false;
  }
  ...
  function expressionKeyForMouth(mouth, eyesClosed = blinkClosed) {
    ...
    if (!eyesClosed && obsExternalInput.emotion) {
      const emotion = obsExternalInput.emotion[0].toUpperCase() + obsExternalInput.emotion.slice(1);
      const emotionKey = `emotion${emotion}${suffix}`;
      if (images[emotionKey]) return emotionKey;
    }
  ...
  function frontHairRootFollowAmount() {
    // 初期キャラは amber-avatar。完成イラストから分離した髪レイヤーなので、
    // サンプル3と同様に頭頂部を顔の剛体変形へ強く追従させる。
  ...
  function computeHeadOffset() {
    ...
    // Character packages are framed left in the editor to leave room for its
    // settings panel.  The desktop mascot has no side panel, so center it in
    // the narrow transparent window instead of inheriting that editor offset.
    const avatarX = DESKTOP_MASCOT_MODE ? 0 : state.avatarX;
    headOffsetCache.x = avatarX + state.angleX * 20 * (state.angleStrength / 100) + swayX;
  ...
  function tick(timestamp) {
    try {
      ...
      applyObsExternalExpression(timestamp);
  ...
  function bindRuntimeControlButtons() {
    ...
    ui.mouseFollowButton?.addEventListener("click", () => {
      if (state.idleMotionEnabled) {
        state.idleMotionEnabled = false;
        idleMotionPlan = null;
        state.mouseFollowEnabled = true;
        syncButtonPressed(ui.idleMotionButton, "待機ゆれ ON", "待機ゆれ OFF", false);
      } else {
      ...
      updateMouseFollowButton();
      markActiveCharacterDirty("settings", "mouse-follow");
    });
