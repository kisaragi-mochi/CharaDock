...
| --- | --- | --- |
| 顔の近くの吹き出しへ返答をストリーミング。入力欄はキャラへマウスを重ねたときだけ表示。 | `会話 / 作業`を小さなUIから切替。会話はキャラごと、作業は直近12件をアプリ終了後も保持。 | Codex Avatar Studioが目・口・表情差分、髪レイヤー、動き、任意指定または自動提案の性格をまとめて生成。 |
...
- `.purupuru`をキャラクタータブから読み込み、画像・表情・髪・調整値・PNGアイテムをアプリ管理領域へ保存
...
- Codex Realtime、ローカルsherpa-onnx、端末音声認識、OpenAI文字起こしから明示的に選択可能
- VADによる無音区切り・認識後の自動送信・3段階の感度調整を選択可能
- Windows標準音声、Style-Bert-VITS2、piper-plus、Supertonic 3、Kokoro、Irodori TTSによる読み上げ、文ごとの表情同期、実音声波形リップシンク
- ユーザー辞書、技術用語辞書、略語、CMUdictの順で英字語を日本語読みし、未登録の一般英単語にも対応（表示文は変更しない）
- キャラクターをクリックしたときの反応は、通常時は選択中のTTSで読み上げ、接続中で応答待ちでないRealtimeでは選択中のLive音声で読み上げ（回答中は重複を避けて表情と吹き出しだけ表示）
- 応答待ちが2.6秒を超えたときだけ、キャラクターらしい短い音声フィラーを一度再生。再生を終えてから回答へつなぐ
- キャラクターごとの触れ合い文、表情、アプリ終了後も残る会話メモリ（最大20往復）

会話履歴とは別に、Codex app-server接続ではキャラクターごとの長期メモリtoolを利用します。各発言から今後も役立つ呼び名・好み・関係性・継続目標を自動判定し、最大24件の小さな利用者プロフィールとして端末内へ保存します。明示的に「覚えて」と言う必要はなく、好みが変わった場合は既存メモリを更新します。一時的な依頼、推測、外部サイトの内容、秘密情報、住所・連絡先・センシティブな属性は保存しません。保存内容は「キャラクター設定」で確認し、個別または一括で削除できます。メモリはキャラクター間で共有されません。

Style-Bert-VITS2は設定の「キャラクター → キャラクターの声」から選択します。ローカルAPIのURL（既定 `http://localhost:5000`）、モデルID、速度だけを指定でき、`/docs`のURLを入力した場合も自動的に`/voice`へ接続します。長い返答は画面へ表示された文ごと（最大90文字）に生成キューへ追加し、最初の文が生成できた時点から表示順に再生します。
...
英単語の日本語読みも同じ設定内で無効化でき、`英字=読み`を1行ずつ登録するユーザー辞書で固有名詞や好みの読みを上書きできます。未登録の一般英単語はCMUdictの英語発音からカタカナ読みを端末内で生成します。画面上の原文は変更せず、読み上げ時だけURL、メールアドレス、ファイルパス、コード、長いハッシュ、Markdown記号などを除外します。リンクは読みやすい表示名があれば表示名だけを読みます。

音声入力は「デスクトップ → 音声入力」で、利用する方式を必ず明示的に選択します。sherpa-onnxでは、日本語Parakeet CTC、ReazonSpeech Zipformer、SenseVoice、Whisper base、従来のWhisper tinyを切り替えられ、各モデルは必要なものだけ初回にダウンロードします。認識言語は日本語へ固定され、録音音声と認識処理は端末内で完結します。VADには約0.6MBのSileroモデルを使用し、準備できない場合だけ従来の音量検出へ戻ります。Codex Realtimeを自動起動したり、失敗時に別方式へ勝手に切り替えたりしません。Realtimeではapp-serverが返す音声一覧からキャラクターごとに声を保存し、GPT-Liveの音声を直接再生します。声の選択肢は男性寄り・女性寄り・中性的という聞こえ方の目安でグループ化し、OpenAI公式の性格説明を日本語で併記します。Realtimeセッションは録音ボタンを押したときだけ開始し、接続中の文字入力も同じLive音声で返します。接続中で回答していないときのキャラクタークリックも同じLive音声を使い、回答中は音声を重ねません。セッション外のクリックは通常TTSへ戻します。この間は通常のTTSを重ねず、少し遅れて届く文字起こしを吹き出しへ逐次表示し、イベント順が前後しても全ターンを会話履歴へ保存します。「キャラクターの声」には現在使う方式を明示し、Liveと通常TTSのうち使わない側を非活性化します。OpenAI文字起こしはAPIへ`ja`を指定し、端末音声認識は`ja-JP`を使用します。感度は「低・標準・高」から選べます。
...
- 作業内容、実行中の操作、完了結果を独立した履歴へ保存し、アプリ再起動後も復元
- 実行中ターンを履歴パネルから中断可能
...
通常の口パクは音量値だけをローカル処理します。sherpa-onnxと端末音声認識はローカルで処理します。Codex RealtimeまたはOpenAI文字起こしを使う場合は、音声が該当サービスへ送られます。現在の入力方式はUIに表示されます。
