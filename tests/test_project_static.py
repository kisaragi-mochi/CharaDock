...
class ProjectStaticTests(unittest.TestCase):
    ...
    def test_public_release_files_exist(self) -> None:
        for relative in [
            ...
            "desktop/control.css",
            "desktop/control.js",
            "desktop/preload-control.cjs",
            "desktop/preload-mascot.cjs",
            "DESKTOP_APP.md",
        ]:
    ...
    def test_bronze_avatar_assets_are_present_and_1254x1254_png(self) -> None:
        bronze_dir = ROOT / "assets" / "bronze-avatar"
        self.assertTrue(bronze_dir.is_dir())
        for filename in [
            "back-hair.png",
            "front-hair.png",
            "eyes-open-mouth-closed.png",
            ...
            self.assertTrue(path.is_file(), filename)
            self.assertEqual(self.png_size(path), (1254, 1254), filename)
    ...
    def test_towa_avatar_assets_are_present_and_1254x1254_png(self) -> None:
        ...
        settings = json.loads((avatar_dir / "default-settings.json").read_text(encoding="utf-8"))
        self.assertEqual(settings["avatarImageSize"], {"width": 1254, "height": 1254})
    ...
    def test_desktop_app_has_secure_electron_boundaries_and_both_backends(self) -> None:
        main = self.read_text("desktop/main.cjs")
        control_preload = self.read_text("desktop/preload-control.cjs")
        package = json.loads(self.read_text("package.json"))
        self.assertEqual(package["main"], "desktop/main.cjs")
        self.assertIn('transparent: true', main)
        self.assertIn('alwaysOnTop:', main)
    ...
    def test_asset_license_keeps_demo_assets_separate(self) -> None:
        ...
        self.assertIn("The software code and documentation text are licensed under [Apache License 2.0](./LICENSE)", asset_license)
        self.assertIn("does not grant rights to the upstream bundled demo avatars", asset_license)
    ...
    def test_gitignore_and_gitattributes_cover_public_cleanup(self) -> None:
        ...
        self.assertNotIn(".agents/", gitignore)
    ...
    def test_development_checks_and_ci_actions_are_pinned(self) -> None:
        ...
        self.assertIn("npm test", readme)
        self.assertIn("npm test", contributing)
        self.assertIn("npm ci", readme)
        self.assertIn("npm ci", contributing)
