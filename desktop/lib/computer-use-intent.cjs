...
const COMPUTER_NOUN = /(?:パソコン|PC|コンピューター|Windows|デスクトップ|アプリ|ウィンドウ|メモ帳|エクスプローラー|設定画面)/i;
const COMPUTER_ACTION = /(?:操作して|操作|動かして|クリックして|押して|入力して|開いて|起動して|切り替えて|設定して|やって)/i;
const STOP_CONTINUATION_PATTERN = /(?:コンピューター|パソコン|PC|Windows|画面)?(?:の)?操作(?:は)?(?:終了|終わり|ここまで)|もう(?:操作しなくて)?いい|ストップ|やめ(?:て|る)/i;
